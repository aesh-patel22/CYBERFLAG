...
// Date filter
$date_filter = isset($_GET['date_filter']) ? filter_input(INPUT_GET, 'date_filter', FILTER_SANITIZE_STRING) : null;
$today = date('Y-m-d', strtotime('2025-09-05')); // Current date for "Today" filter
...
try {
    $stmt = $pdo->query("SELECT DISTINCT DATE(created_at) AS creation_date FROM tbl_user_detail ORDER BY creation_date DESC");
    $dates = $stmt->fetchAll(PDO::FETCH_COLUMN);
} catch (PDOException $e) {
    error_log("Error fetching dates: " . $e->getMessage());
    $feedback = "Error: Could not fetch creation dates.";
    $dates = [];
}

// Fetch total score count
try {
    $count_query = "SELECT COUNT(*) FROM tbl_user_score us 
                    LEFT JOIN tbl_user_detail ud ON us.user_id = ud.user_id";
    if ($date_filter) {
        $count_query .= $date_filter === 'today' ? " WHERE DATE(ud.created_at) = :today" : " WHERE DATE(ud.created_at) = :date";
    }
...
// Fetch scores with average time
try {
    $query = "SELECT us.user_id, ud.user_name, us.total_point, 
              ...
              LEFT JOIN tbl_user_detail ud ON us.user_id = ud.user_id";
    if ($date_filter) {
        $query .= $date_filter === 'today' ? " WHERE DATE(ud.created_at) = :today" : " WHERE DATE(ud.created_at) = :date";
    }
    $query .= " ORDER BY us.user_id ASC LIMIT :limit OFFSET :offset";
    ...
    // Calculate average time for each user
    $scores = [];
    ...
    $feedback = "Error: Could not fetch scores. Ensure tables tbl_user_score and tbl_user_detail exist and are accessible.";
    $scores = [];
}
...
<head>
    ...
    <title>HACKERSTORM :: View Scores</title>
...
<body>
    ...
    <div class="main-content">
        <div class="max-w-7xl mx-auto">
            <div class="flex items-center justify-between mb-6">
                <h1 class="text-2xl font-bold text-white">View Scores</h1>
            ...
            <div class="mb-6 flex flex-col sm:flex-row gap-4">
                ...
                <div class="relative date-filter-container">
                    ...
                    <select id="dateFilter" class="date-filter-select font-mono">
                        <option value="">All Dates</option>
                        <option value="today" <?php echo $date_filter === 'today' ? 'selected' : ''; ?>>Today (<?php echo $today; ?>)</option>
            ...
            <div class="card p-8">
                <table class="w-full text-left">
                    ...
                    <tbody id="scoreTableBody">
                        ...
                        <tr class="table-row no-scores">
                            <td colspan="6" class="p-5 text-center text-gray-300">No scores found.</td>
                        </tr>
                        <?php else: ?>
                        <?php $sr_no = $offset + 1; ?>
                        <?php foreach ($scores as $score): ?>
                        <tr class="table-row border-b border-gray-600/50" data-username="<?php echo htmlspecialchars($score['user_name'] ?? 'N/A', ENT_QUOTES, 'UTF-8'); ?>">
                ...
                <div class="mt-6 flex justify-between items-center">
                    <div class="flex items-center space-x-3">
                        ...
                        <select class="pagination-select font-mono" onchange="window.location.href='?page=1&per_page='+this.value<?php echo $date_filter ? '&date_filter='.urlencode($date_filter) : ''; ?>">
                    ...
                    <div class="flex space-x-2">
                        <a href="?page=<?php echo max(1, $page - 1); ?>&per_page=<?php echo $per_page; ?><?php echo $date_filter ? '&date_filter='.urlencode($date_filter) : ''; ?>" class="pagination-button <?php echo $page <= 1 ? 'opacity-50 cursor-not-allowed' : ''; ?>" <?php echo $page <= 1 ? 'disabled' : ''; ?>>
                            <i class="fas fa-chevron-left mr-2"></i> Previous
                        </a>
                        <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                        <a href="?page=<?php echo $i; ?>&per_page=<?php echo $per_page; ?><?php echo $date_filter ? '&date_filter='.urlencode($date_filter) : ''; ?>" class="pagination-button <?php echo $page == $i ? 'bg-primary-600' : ''; ?>">
                            <?php echo $i; ?>
                        </a>
                        <?php endfor; ?>
                        <a href="?page=<?php echo min($total_pages, $page + 1); ?>&per_page=<?php echo $per_page; ?><?php echo $date_filter ? '&date_filter='.urlencode($date_filter) : ''; ?>" class="pagination-button <?php echo $page >= $total_pages ? 'opacity-50 cursor-not-allowed' : ''; ?>" <?php echo $page >= $total_pages ? 'disabled' : ''; ?>>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const rows = scoreTableBody.querySelectorAll('.table-row:not(.no-scores)');
            ...
            searchInput.addEventListener('input', function() {
                ...
                const visibleRows = scoreTableBody.querySelectorAll('.table-row:not([style*="display: none"])');
                ...
                const noScoresRow = scoreTableBody.querySelector('.no-scores');
                if (visibleRowCount === 0 && !noScoresRow) {
                    const noScoresRow = document.createElement('tr');
                    noScoresRow.className = 'table-row no-scores';
                    noScoresRow.innerHTML = '<td colspan="6" class="p-5 text-center text-gray-300">No scores found.</td>';
                    scoreTableBody.appendChild(noScoresRow);
            ...
            dateFilter.addEventListener('change', function() {
                window.location.href = `?page=1&per_page=${<?php echo $per_page; ?>}&date_filter=${this.value}`;
            });

            // Modal open/close
            function openDetailsModal(userId) {
                fetch('view_score.php?get_details=' + userId)
                    ...
                    .then(data => {
                        ...
                        modalContent.innerHTML = `
                            ...
                            <div class="mb-4 flex items-center gap-3">
                                ...
                                <div class="flex-1">
                                    <label class="block text-gray-300 text-sm font-semibold mb-2">User Created At</label>
                                    <input type="text" value="${data.user_created_at || 'N/A'}" class="modal-input w-full font-mono" readonly>
                                </div>
                            </div>
                            <div class="mb-4 flex items-center gap-3">
                                <i class="fas fa-clock text-primary-400"></i>
